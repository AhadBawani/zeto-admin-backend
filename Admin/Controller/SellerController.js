...
module.exports.GET_ALL_SELLER = (async (req, res) => {
    try {
        ...
        for (let i = 0; i < seller.length; i++) {
            const sellerReview = await SellerReviewSchema.find({ sellerId: seller[i]?._id, deleteReview:false }).select('_id sellerId date userId rating review').populate('userId', 'email username phoneNumber');
