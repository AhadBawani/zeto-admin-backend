...
module.exports.GENERATE_ORDER_PDF = (async (req, res) => {
    ...
    try {
        const order = Order.find({ orderId: orderId, userId: userId })
            ...
            .then(response => {
                if (response.length > 0) {
                    ...
                    doc.fontSize(16).text(`Order ID: ${order.orderId}`);
                    doc.fontSize(16).text(`User: ${order.userId?.username}`);
                    // doc.fontSize(16).text(`Order Date: ${order.createdAt.toDateString()}`);
                    doc.moveDown();
